...
import { getCursos, updateCurso } from "@/model/api/backend/apiCursos"
...
export function EditCursoForm({
  ...
  const onSubmit: SubmitHandler<ICurso> = async (data) => {
    ...
    try {
      await updateCurso(cursoSeleccionado.curso_id, {
        titulo: data.titulo,
    ...
    <div className={cn("flex flex-col gap-8 items-center w-full", className)} {...props}>
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="text-balance text-muted-foreground max-w-md">
          {cursoSeleccionado
            ? "Modifica la informacion en los campos de abajo y guarda los cambios correspondientes."
      ...
      {message && (
        <div
          className={cn(
            "p-4 rounded-lg text-center font-semibold w-full max-w-2xl",
            ...
            <CardContent className="grid p-0 md:grid-cols-2">
              <FieldGroup className="gap-4 w-130">
                <Field>
                  ...
                  <Input id="edit-titulo" type="text" placeholder="Titulo del curso" required {...register("titulo")} />
                </Field>
                <Field>
                  <FieldLabel htmlFor="edit-categoria">Categoria</FieldLabel>
                  <Input id="edit-categoria" type="text" placeholder="Categoria del curso" required {...register("categoria")} />
                </Field>
                <Field>
                  <FieldLabel htmlFor="edit-academia">Academia</FieldLabel>
                  <Input id="edit-academia" type="text" placeholder="Academia del curso" required {...register("academia")} />
                </Field>
                <Field>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b text-muted-foreground text-sm font-semibold">
                  <th className="pb-3 pl-2">Titulo</th>
              ...
              <tbody className="divide-y text-sm">
                {cursos.map((curso) => (
                  <tr key={curso.curso_id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="py-3 text-right font-semibold">{curso.precio} EUR</td>
