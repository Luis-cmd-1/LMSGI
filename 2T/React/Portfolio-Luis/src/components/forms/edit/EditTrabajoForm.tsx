...
import { updateTrabajo } from "@/model/api/backend/apiTrabajos"
import { useForm } from "react-hook-form"
import type { SubmitHandler } from "react-hook-form"
import { useState } from "react"

export function EditTrabajoForm({
  item,
  onSuccess,
}: {
  item: ITrabajo
  ...
  const { register, handleSubmit } = useForm<ITrabajo>({ defaultValues: item })
  ...
  const onSubmit: SubmitHandler<ITrabajo> = async (data) => {
    ...
    setMessage(null)
    try {
      await updateTrabajo(item.trabajo_id, data)
  ...
  return (
    <div className={cn("flex w-full flex-col gap-6 items-center")}>
      <div className="flex flex-col items-center gap-2 text-center">
        <h1 className="text-2xl font-bold">Editar Trabajo</h1>
          ...
          <CardContent className="p-0">
            <FieldGroup className="w-full gap-4">
              <Field>
                ...
                <Input id="titulo" type="text" placeholder="Titulo del trabajo" required {...register("titulo")} />
              </Field>
              <Field>
              ...
              <Field>
                <FieldLabel htmlFor="tecnologias">Tecnologias (separadas por coma)</FieldLabel>
