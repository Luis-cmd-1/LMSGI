...
- **Rutas de admin (envueltas en `BackendLayout`)**: Dashboard (`/admin`, con `index`), `/admin/Cursos`, `/admin/Formacion`, `/admin/Servicios`, `/admin/Trabajos` — todas empiezan con `/admin`, lo que las hace "inaccesibles" desde la navegación normal (no hay enlaces públicos a ellas, solo aparecen en el menú del panel de administración). La ruta `index` hace que el Dashboard se muestre al entrar a `/admin` sin necesidad de escribir nada más.
...
## 4. Animaciones de transición entre páginas

### ¿Cómo funciona la animación al cambiar de página?

El proyecto tiene **dos animaciones de transición**, una para la parte pública y otra para el panel de admin. Ambas están definidas en `src/index.css`:

```css
@keyframes page-enter {
  from {
    opacity: 0;
    transform: translateX(30px);  /* Viene desde la derecha */
  }
  to {
    opacity: 1;
...
.page-enter-back {
  animation: page-enter-back 0.5s ease-out;
}
```

- **Frontend (`MainLayout`)**: Usa `page-enter` — la página entra deslizándose desde la derecha con un fundido.
- **Backend (`BackendLayout`)**: Usa `page-enter-back` — la página entra deslizándose desde la izquierda, para diferenciar visualmente la zona de administración.

Se activan gracias a `key={transicion.pathname}` en el `div` que envuelve al `Outlet`. Cuando la ruta cambia, React detecta que la `key` ha cambiado y recrea el `div`, disparando la animación CSS.

---

## 5. Cada página explicada individualmente

### 5.1. Home (`src/pages/Home.tsx`) — Página principal
...
### 5.2. Formación (`src/pages/formacion/Formacion.tsx`)
...
Los datos están definidos **directamente en el código** como un array de objetos.
...
### 5.3. Servicios (`src/pages/servicios/Servicios.tsx`)
...
### 5.4. Cursos (`src/pages/cursos/Curso.tsx`)
...
### 5.5. Trabajos (`src/pages/trabajos/Trabajos.tsx`)
...
### 5.6. Contacto (`src/pages/Contacto.tsx`)
...
Además, el botón de enviar está deshabilitado porque el checkbox de privacidad está comentado.

⚠️ **A mejorar:** El formulario de contacto no es funcional — no envía correos.

---

### 5.7. Dashboard (`src/pages/admin/Dashboard.tsx`) — Página de inicio del panel de admin

**¿Qué muestra?**
- Un **hero** de bienvenida con el texto "Bienvenido al panel de administración" y una descripción.
- Una sección con **3 tarjetas de acciones** (Insertar, Editar, Eliminar) con colores verde, azul y rojo respectivamente.
- Una parrilla con **4 enlaces** a las secciones del admin: Cursos, Formación, Servicios, Trabajos.

**¿Cómo está construido?**
Los datos de las secciones se definen en un array constante:
...
Y se recorren con `.map()` para pintar cada enlace. Esto hace que añadir una nueva sección solo requiera añadir un objeto al array.

Los iconos se importan de **Lucide React**, la misma librería de iconos que usa el panel de admin.
...
## 6. Los componentes reutilizables

### 6.1. Header (`src/components/main/Header.tsx`)
...
### 6.2. Footer (`src/components/main/Footer.tsx`)
...
### 6.3. CursosCard + CursoCard (`src/components/cursos/`)
...
### 6.4. TrabajosCard + TrabajoCard (`src/components/trabajos/`)
...
### 6.5. FormacionCard (`src/components/formacion/FormacionCard.tsx`)

Misma estructura que TrabajoCard pero para datos de formación: categoría, título, autor, fecha, descripción expandible.

### 6.6. ServicioCard (`src/components/servicios/ServicioCard.tsx`)

Muestra tipo, nombre, descripción expandible, características y precio.

### 6.7. Componentes "Wrapper" (`CursosCard`, `TrabajosCard`, `FormacionesCard`, `ServiciosCard`)
...
### 6.8. Componentes UI de shadcn (`src/components/ui/`)
...
## 7. Los `.map()` explicados uno a uno ⭐

Aquí están **todos** los `.map()` del proyecto, en orden de aparición:

### 7.1. Home.tsx — Stack técnico (línea 85)
...
### 7.5. Formacion.tsx — Skills dentro de cada tarjeta (línea 163)
...
### 7.6. Servicios.tsx — Stats (línea 118)

Mismo patrón: "6" → "Servicios", "4" → "Disponibles", "2+" → "Años exp."

### 7.7. Servicios.tsx — Tarjetas de servicios (línea 140)
...
### 7.8. Servicios.tsx — Skills dentro de cada tarjeta (línea 186)
...
### 7.10. TrabajosCard.tsx — (línea 11)
...
### 7.11. TrabajoCard.tsx — Tecnologías (línea 52)
...
### 7.12. FormacionesCard.tsx — (línea 11)
...
### 7.13. ServiciosCard.tsx — (línea 11)
...
### 7.14. Header.tsx — Enlaces de navegación (línea 57 y 131)
...
### 7.15. DeleteCursoForm.tsx — Lista de cursos en el select (línea 68)
...
### 7.16. UpdateCursoForm.tsx — Tabla de cursos (línea 193)
...
### 7.17. Dashboard.tsx — Tarjetas de secciones (línea 57)

```tsx
{SECCIONES.map((s) => (
  <Link key={s.label} to={s.ruta} ...>
    <s.icon />
    <h2>{s.label}</h2>
    <p>{s.descripcion}</p>
  </Link>
...
- **Array recorrido:** `SECCIONES`, un array de 4 objetos con label, descripción, icono y ruta.
- **Qué devuelve:** Un `<Link>` por cada sección del admin. Cada enlace tiene un icono, el nombre de la sección y una descripción breve.
- **Analogía:** "Tengo 4 secciones en el panel de admin, y por cada una pinto una tarjeta que enlaza a su página."

### 7.18. Dashboard.tsx — Tarjetas de acciones (línea 40)
...
- **Array recorrido:** `ACCIONES`, un array de 3 objetos con label, descripción, icono y color.
- **Qué devuelve:** Una tarjeta informativa por cada acción (Insertar, Editar, Eliminar) con un color distintivo.

### 7.19. NavMain.tsx — Items del menú lateral (línea 45)
...
### 7.20. NavSecondary.tsx — Items secundarios (línea 27)
...
### 7.21. NavDocuments.tsx — Items de documentos (línea 35)

Actualmente este componente devuelve `null` porque la sección "Documentos" está comentada.

---

## 8. La conexión con Supabase (la "base de datos") ⭐ y el sistema Mock
...
- `VITE_SUPABASE_URL` es la URL del proyecto en Supabase.
...
#### 8.1. Cursos — Leer todas (`apiCursos.ts`, línea 5)
...
#### 8.2. Cursos — Insertar (`apiCursos.ts`, línea 15)
...
#### 8.3. Cursos — Eliminar (`apiCursos.ts`, línea 28)
...
#### 8.4. Cursos — Obtener por ID (`apiCursos.ts`, línea 41)
...
#### 8.5. Cursos — Actualizar (`apiCursos.ts`, línea 54)
...
#### 8.6. Formaciones — Leer todas (`apiFormacion.ts`)
...
#### 8.7. Formaciones — Insertar (`apiFormacion.ts`, línea 15)

Mismo patrón que `insertCurso` pero con la tabla `formaciones`.

#### 8.8. Servicios — Leer todas (`apiServicios.ts`)

Mismo patrón, tabla `servicios`.

#### 8.9. Servicios — Insertar (`apiServicios.ts`, línea 15)

Mismo patrón.

#### 8.10. Trabajos — Leer todos (`apiTrabajos.ts`)

Mismo patrón, tabla `trabajos`.

#### 8.11. Trabajos — Insertar (`apiTrabajos.ts`, línea 15)
...
## 9. El panel de administración (Admin CRUD)
...
### 9.1. La barra lateral del admin (AppSidebar)

La barra lateral del panel de administración está construida con varios componentes:

| Componente | Propósito |
|-----------|-----------|
| `AppSidebar` | Contenedor principal de la sidebar |
| `NavMain` | Lista de enlaces a las secciones del admin |
| `NavSecondary` | Enlaces secundarios (Salir) |
| `NavDocuments` | Sección de documentos (actualmente vacía) |
| `NavUser` | Avatar e información del usuario |

#### Elementos visibles en la sidebar:

- **Header**: Muestra el icono `LayoutDashboardIcon` con el texto "Dashboard" en fondo negro, enlazando a `/admin`. Es el botón más destacado de la sidebar.
- **NavMain**: Lista los 4 enlaces a las secciones: Formación (`GraduationCapIcon`), Servicios (`HandPlatterIcon`), Trabajos (`BriefcaseBusinessIcon`), Cursos (`FolderIcon`).
- **Salir**: Enlace a la página principal (`/`) con un ligero fondo rojo (`bg-red-500/10`) que se intensifica al pasar el ratón.

Los iconos se importan de la librería **Lucide React**, que ofrece iconos SVG en formato de componentes React.

El texto de los enlaces usa `text-base` y hay un espacio de `gap-3` entre elementos para mejorar la legibilidad.

#### Componentes comentados (no visibles):
- **Quick Create**: Era un botón destacado en la parte superior de NavMain. Se eliminó porque el Dashboard en el header cumple esa función.
- **Documentos y More**: Sección de documentos con acciones desplegables. Está comentada porque no tiene funcionalidad real en el proyecto.
- **Ajustes, Ayuda, Buscar**: Enlaces secundarios comentados.

### 9.2. Admin Cursos (`/admin/Cursos`) — El más completo
...
### 9.6. Dashboard (`/admin`) — Página de inicio del admin

La página de Dashboard se creó como ruta `index` dentro del `BackendLayout`. Proporciona:
- Un resumen visual del panel de administración
- Acceso rápido a cada sección mediante tarjetas enlazables
- Una descripción de las acciones posibles (Insertar, Editar, Eliminar)

Está construida con el mismo sistema de componentes y estilos que el resto del admin.
...
- **Sección de Salir** en la parte inferior con estilo rojo.
- **Header superior** con título "Documentos".
- **Avatar de usuario** con menú desplegable.

⚠️ **A mejorar:** El panel de admin **no tiene autenticación**. Cualquiera que sepa la URL `/admin` puede acceder.

---

## 10. El sistema de estado con useState y useEffect ⭐
...
const [form, setForm] = useState({ nombre: '', apellido: '', email: '', mensaje: '' })
```
- **Qué guarda:** Los datos del formulario de contacto.
...
## 11. TypeScript: los tipos e interfaces
...
## 12. El diseño visual: Tailwind CSS y shadcn/ui
...
En `src/index.css` se definen variables CSS para el tema, usando el formato `oklch` (un espacio de color moderno). Luego, en `@theme inline` se mapean estas variables a nombres de clase de Tailwind, permitiendo usar clases como `bg-background`, `text-foreground`, `border-border`, etc.

### Animaciones CSS

El proyecto usa **`tw-animate-css`**, una librería que proporciona clases de animación para Tailwind. Ejemplos de su uso:

- **Transiciones de página**: `page-enter` (frontend, slide desde la derecha) y `page-enter-back` (backend, slide desde la izquierda), definidas con `@keyframes` en `index.css`.
- **Punto parpadeante (`animate-pulse`)**: En las tarjetas de cursos, servicios y trabajos, un pequeño punto verde que parpadea para indicar disponibilidad.
- **Línea de barrido (`scan`)**: Una línea horizontal que recorre la pantalla cada 4 segundos con un efecto "cyberpunk".
...
## 13. Preguntas que me pueden hacer y sus respuestas
...
- **shadcn/ui** — para tener componentes accesibles y bonitos (botones, tarjetas, inputs, sidebar).
- **react-hook-form** — para manejar formularios de forma más limpia en el panel de admin.
- **Lucide React** — para iconos en el panel de administración y la web.
- **Sonner** — para notificaciones (aunque parece no estar usado aún).
- **tw-animate-css** — para animaciones CSS con Tailwind.
...
### P23: ¿Qué animaciones tiene el proyecto?

**Respuesta:** El proyecto tiene dos tipos principales de animaciones:
1. **Transiciones de página**: Al navegar entre páginas, el contenido se desliza y funde. En la parte pública entra desde la derecha, y en el panel de admin desde la izquierda para diferenciar visualmente ambas zonas.
2. **Punto parpadeante**: Las tarjetas de disponibilidad tienen un punto verde que parpadea para indicar "activo".
3. **Línea de barrido**: Una línea horizontal cruza la pantalla periódicamente, dando un efecto técnico/cyberpunk.

### P24: ¿Qué librería de iconos usas y por qué?

**Respuesta:** Uso **Lucide React**, que proporciona iconos SVG en formato de componentes React. Elegí Lucide porque es la librería que usa shadcn/ui por defecto, lo que garantiza consistencia visual. He usado iconos como `LayoutDashboardIcon`, `GraduationCapIcon` (formación), `HandPlatterIcon` (servicios), `BriefcaseBusinessIcon` (trabajos), y `FolderIcon` (cursos).
              ...
              └── App.tsx
                   └── AppRouter.tsx
                        ...
                        └── BackendLayout (Sidebar + Header + Outlet)
                             ├── Dashboard (/admin) — página de inicio
...
| **Transición CSS** | Animación que se activa al cambiar de página, dando sensación de fluidez |
| **Lucide React** | Librería de iconos en formato de componentes React |
